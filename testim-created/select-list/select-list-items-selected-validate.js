...
/* globals element, matchType, checkState, Event, setTimeout, document, itemId, customListSelectors, MutationObserver, targetElementCSS, document */
...
function validateSelectedItems(actualValues, expectedValues, matchType) {
...
copyToClipboard(JSON.stringify(actualSelectedItems?.selected_options, null, 1));
exportsTest[return_variable_name] = actualSelectedItems?.selected_options;
if (verbose)
    console.log("Starting element tagName", element.tagName);

if (verbose) {
    console.log("actualSelectedItems", JSON.stringify(actualSelectedItems?.selected_options));
...
if (typeof expectedValues !== 'undefined' && expectedValues !== null) {
    ...
    validateSelectedItems(actualSelectedItems?.selected_options, expected_values, match_type);
